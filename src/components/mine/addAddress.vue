...
<div class="page page-current" style="background:#FFF;">
  ...
  <div style="margin: 44px auto;">
    ...
    <div style="height: 48px;width: 100%;background: #ffffff;border-bottom:1px solid #EEE;">
      <span style="float: left;
        ...
        <label for="chk" style="float: right;height: 48px;font-size: 14px;line-height:52px;padding:0 25px;">
          <img v-if="chk?true:false" src="http://download.dl.quzhuan.me/image/sdk/h5/right@3x.png" style="width:15px;" alt="">
          <span v-if="!chk?true:false" style="width:15px;height:15px;border:1px solid #777;border-radius:50%;display:inline-block;"></span>
        </label>
    ...
    <p style="padding: 8px 20px; color:#777777;font-size:13px; ">(实物收货地址用户实物奖品中奖后发货,最多可添加三个收货地址,QQ号码填写用于Q币等虚拟商品中奖后充值使用)</p>
  ...
  methods: {
    ...
    console.log(this.ADarea);
       ...
       if (window.localStorage.getItem('adname')) {
         this.$http.post('http://123.59.49.17:8080/platform/api/v1/user/address/update', {}, {
           params: {
             token: this.token,
             id: this.ADid,
             name: this.ADname,
         ...
         this.$http.post('http://123.59.49.17:8080/platform/api/v1/user/save/address', {}, {
           params: {
             token: this.token,
             name: this.ADname,
             mobile: this.ADphone,
