		...
		<div style="background: #FFFFFF;border-radius: 8px; width: 80%; height: 275px; margin: 143px 10%;">
			<div style="text-align: center;background: #EEEEEE; border-radius: 8px 8px 0 0;
				...
				<span style="float: right; margin-right: 16px;" @click='close1'>
					 <img src="../../../static/img/quxiao.png" alt="" style="width: 16px; height: 16px;"/>
				 </span>
			</div>
	...
	<div class="row no-gutter sort" :class="{'fixed':isFixed}">
		...
		<div class="col-50" @click="tabPrice">
			<span :class="{'redColor':isPrice}" style="display: inline-block;width: 32%;position:relative;">
				价格
				<span class="iconfont icon-shang" style="position:absolute;top:-5px;font-size:14px;" :style="isBig?{color:'#D43047'}:{color:'#777'}"></span>
	...
	<div class="row no-gutter goods">
		<div class="col-50" v-for="x in goods">
			<div class='haha' v-link="{name:'details', params:{number:x.number,from:from}}" style="position:relative;">
				<img src="../../../static/img/10p.png" alt="10" style="width:20%;position:absolute;top:10px;left:10px;" v-show="x.region==10?true:false" />
...
export default {
	data() {
		return {
			...
			tenB: false, // 是否为十元商品
			currentPay: 5, // 当前购买数量
			appNumber: window.localStorage.getItem('appNumber') ? window.localStorage.getItem('appNumber') : this.$route.query.appNumber,
			...
			slides: [], // 轮播data
			banner_titles: [{
				img: './static/img/ten.png',
				title: '十元专区',
				link: '/home/tenGoods'
			}, {
				img: './static/img/open.png',
				title: '秒开商品',
				link: '/home/secondK'
			}, {
				img: './static/img/announce.png',
				title: '即将揭晓',
				link: '/home/coming'
			}, {
				img: './static/img/help.png',
				title: '帮助中心',
				link: '/message'
			}], // banner下的选项区
			horn: [], // 喇叭data
	...
	methods: {
		...
		xrQQ: function() {
			if (window.localStorage.getItem('token')) {
				// 有token的情况
				this.$http.get('http://123.59.49.17:8080/platform/api/v1/index/pop/check ', {
					params: {
						token: this.token
					}
				}).then(
		...
		tabPopularity: function() {
			...
			this.isPrice = false;
			this.isBig = true;
			this.$http.get('http://123.59.49.17:8080/platform/api/v1/index/goods/list', {
				params: {
					appNumber: this.appNumber,
					field: 'hot',
					page: '1'
	...
	created() {
		// 获取数据
		this.$http.get('http://123.59.49.17:8080/platform/api/v1/index/banner/list', {
			params: {
				appNumber: this.appNumber,
				type: '1',
				device: '4'
		...
		this.$http.get('http://123.59.49.17:8080/platform/api/v1/index/roll/show').then(
			function(res) {
	...
	ready() {
		...
		var that = this;
		var page = 1;
		if (this.$route.query.appNumber) {
			this.appNumber = this.$route.query.appNumber;
			window.localStorage.setItem('appNumber', this.appNumber);
		}
		//		this.appNumber = this.$route.query.appNumber;
		...
		$('.content').on('scroll', function() {
			if (this.scrollTop >= (this.scrollHeight - this.clientHeight - 600)) {
				...
				if (that.isPopularity === true && page <= 10) {
					that.$http.get('http://123.59.49.17:8080/platform/api/v1/index/goods/list', {
						params: {
